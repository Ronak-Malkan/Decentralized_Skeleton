...
double Server::computeScore() {
    ...
    if (sysctlbyname("kern.boottime",&boottime,&len,nullptr,0)!=0) boottime.tv_sec=0;
...
void Server::listenerLoop() {
    ...
    class ServiceImpl final : public Mini3Service::Service {
        ...
        FileLogger&              logger_;
    public:
        ServiceImpl(ThreadSafeQueue<Message>& q, FileLogger& log)
          : queue_(q), logger_(log) {}
        ...
        Status SubmitTask(ServerContext*, const TaskRequest* req, TaskResponse*) override {
            Message m{MessageType::TASK_REQUEST, req->reply_to(), {}, *req};
            queue_.push(std::move(m));
            return Status::OK;
        }
    };

    ServiceImpl service{inbound_, *logger_};
...
void Server::processLocal(const Message& msg) {
    logger_->log("task_start", {{"task", msg.task_request.task_id()}});
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    TaskResponse resp;
    resp.set_task_id(msg.task_request.task_id());
    resp.set_result("Processed by " + node_id_);
    // TODO: send back to client via msg.task_request.reply_to()
    logger_->log("task_complete", {{"task", msg.task_request.task_id()}});
}
...
void Server::forwardTask(const Message& msg) {
    ...
    for (auto& [id,e] : peer_info_)
        if (e.score>bestScore) { bestScore=e.score; bestId=id; }

    auto viaIt = next_hop_.find(bestId);
    if (viaIt==next_hop_.end()) {
        logger_->log("task_drop", {{"task", msg.task_request.task_id()}});
        return;
    }
    const auto& via = viaIt->second;
    logger_->log("task_forward", {
        {"task", msg.task_request.task_id()},
        {"to",   via}
    });

    auto it = std::find(stub_addrs_.begin(), stub_addrs_.end(), via);
    if (it==stub_addrs_.end()) return;
    size_t idx = std::distance(stub_addrs_.begin(), it);

    // Immediately send on its own detached thread
    std::thread([this, idx, via, msg]() {
        ClientContext ctx;
        ctx.set_deadline(std::chrono::system_clock::now()
                         + std::chrono::milliseconds(200));
        TaskResponse resp;
        Status s = stubs_[idx]->SubmitTask(&ctx, msg.task_request, &resp);
        if (s.ok()) {
            logger_->log("task_forward_ok", {
                {"task", msg.task_request.task_id()},
...
void Server::heartbeatLoop() {
    ...
    while (running_) {
        ...
        // (we’ll re-enable gossip fields later)
...
void Server::metricsLoop() {
    ...
    while (running_) {
        ...
        if(getloadavg(&load,1)!=-1){
            ...
            double u=c?(load/c):load;
            u=std::clamp(u,0.0,(double)c);
            load=(1.0-u/c)*100.0;
        }
        double mem_free_pct=50.0;
        ...
        size_t q=inbound_.size();
        double s=computeScore();
